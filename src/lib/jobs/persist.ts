...
export async function persistCandidateJob(runId: string, candidate: CandidateJob) {
  ...
  if (existingJob) {
    ...
    job = await db.update(schema.jobs).set({
      ...
      experienceMinOriginal: candidate.experience?.minYears ?? existingJob.experienceMinOriginal,
      experienceMaxOriginal: candidate.experience?.maxYears ?? existingJob.experienceMaxOriginal,
      descriptionOriginal: candidate.description ? JSON.stringify(candidate.description) : existingJob.descriptionOriginal,
      candidateRemoteEligibility: candidate.job.candidateRemoteEligibility || existingJob.candidateRemoteEligibility,
      geographicRemoteScope: candidate.job.geographicRemoteScope || existingJob.geographicRemoteScope,
      geographicEligibilityReason: candidate.job.geographicEligibilityReason || existingJob.geographicEligibilityReason,
      geographicEligibilityConfidence: candidate.job.geographicEligibilityConfidence || existingJob.geographicEligibilityConfidence,
    ...
    job = await db.insert(schema.jobs).values({
      ...
      experienceMinOriginal: candidate.experience?.minYears,
      experienceMaxOriginal: candidate.experience?.maxYears,
      descriptionOriginal: candidate.description ? JSON.stringify(candidate.description) : null,
      candidateRemoteEligibility: candidate.job.candidateRemoteEligibility,
      geographicRemoteScope: candidate.job.geographicRemoteScope,
      geographicEligibilityReason: candidate.job.geographicEligibilityReason,
      geographicEligibilityConfidence: candidate.job.geographicEligibilityConfidence,
      experienceMin: candidate.experience?.minYears,
      experienceMax: candidate.experience?.maxYears,
      description: candidate.description ? JSON.stringify(candidate.description) : null,
      status: candidate.job.status,
      firstSeenAt: now,
      lastSeenAt: now,
      createdAt: now,
